/**
 * 定时数据更新脚本 — 由 GitHub Actions 调用
 * 执行顺序：RSS 抓取 → DeepL 翻译 → DeepSeek 分析 → Polymarket → 行情 → 写入 Vercel KV
 */
...
async function fetchAndRefresh() {
  ...
  const translatedTitles = await translateBatch(rawNew.map(a => a.titleEn), DEEPL_TOKEN);
...
async function generateSituationReport() {
  if (!DEEPSEEK_TOKEN) { console.warn('[分析] DEEPSEEK_API_TOKEN 未设置，跳过'); return; }

  const events = (await kv.get('events')) || [];
  if (!events.length) { console.warn('[分析] 无事件，跳过'); return; }
  ...
  console.log(`[分析] 调用 DeepSeek（基于 ${sample.length} 条事件）...`);
  try {
    const res = await fetch('https://api.deepseek.com/chat/completions', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${DEEPSEEK_TOKEN}`, 'Content-Type': 'application/json' },
    ...
    if (!res.ok) { console.error(`[分析] DeepSeek HTTP ${res.status}`); return; }

    const json    = await res.json();
    const content = json.choices?.[0]?.message?.content;
    if (!content) { console.error('[分析] 空响应'); return; }

    const report = JSON.parse(content);
    const result = { ...report, lastUpdated: new Date().toISOString(), basedOnEvents: sample.length };
    await kv.set('analysis', result);
