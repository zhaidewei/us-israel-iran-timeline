...
async function fetchAndRefresh(store, { deeplToken, deepseekToken } = {}) {
  ...
  const textsToTranslate = [
    ...rawNew.map(a => a.titleEn),
    ...rawNew.map(a => a.summaryEn)
  ];
  const translatedTexts = await translateBatch(textsToTranslate, deeplToken);
  const n = rawNew.length;
  const translated = rawNew.map((a, i) => ({
    ...a,
    titleZh:   translatedTexts[i]     || a.titleEn,
    summaryZh: translatedTexts[n + i] || ''
  }));
