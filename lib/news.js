...
const parser = new Parser({
  ...
  customFetch: (url, opts) => fetch(url, { ...opts, compress: true, headers: { ...opts?.headers, 'User-Agent': BROWSER_UA } })
});
...
const RSS_FEEDS = [
  ...
  { name: '纽约时报',     nameKey: 'nyt',      url: 'https://rss.nytimes.com/services/xml/rss/nyt/MiddleEast.xml' },
  { name: 'NPR',          nameKey: 'npr',      url: 'https://feeds.npr.org/1004/rss.xml' },
  { name: 'TASS',         nameKey: 'tass',     url: 'https://tass.com/rss/v2.xml' },
  { name: '环球时报',     nameKey: 'globaltimes', url: 'https://www.globaltimes.cn/rss/outbrain.xml' },
  { name: 'CGTN',         nameKey: 'cgtn',     url: 'https://www.cgtn.com/subscribe/rss/section/world.xml' },
];
...
function eventDedupKey(e) {
  ...
  if (linkKey) return `${e.source || ''}|${linkKey}`;
  return `${e.source || ''}|${e.titleEn || ''}|${e.pubDate || ''}`;
}
...
async function analyzeChunk(articles, clusterRef = new Map(), deepseekToken) {
  ...
  const prompt =
    ...
    `每篇文章返回：\n` +
    `1. titleZh：标题的简体中文翻译\n` +
    `2. eventCluster：事件聚类 key（英文，格式：主题-月日，如 "us-iran-strike-0228"；同一事件复用已有 key）\n` +
    `3. category：军事打击 / 防空拦截 / 外交动向 / 人员伤亡 / 制裁经济 / 内政局势 / 其他\n` +
    `4. importance：1-5（5=重大突发，4=重要，3=常规，2=背景，1=次要）\n` +
    `5. briefZh：30-50字中文战报摘要，简洁客观；信息不足时留空字符串\n\n` +
    `新闻列表：\n${list}\n\n` +
    `严格返回（数组长度必须为 ${articles.length}）：\n` +
    `{"results":[{"titleZh":"...","eventCluster":"...","category":"...","importance":3,"briefZh":"..."},...]}`;
  ...
  try {
    ...
    return articles.map((a, i) => ({
      ...a,
      titleZh:      results[i]?.titleZh      || a.titleEn,
 ...
 * @param {{ deepseekToken?: string }} tokens
 */
async function fetchAndRefresh(store, { deeplToken, deepseekToken } = {}) {
  console.log('\n[新闻] 开始获取 RSS...');
  const existingRaw = (await store.get('events')) || [];
  const existingEvents = dedupeEvents(existingRaw);
  const existingIds = new Set(existingEvents.map(e => e.id));
  const existingKeys = new Set(existingEvents.map(eventDedupKey));
  const rawNew = [];
  ...
  for (const feed of RSS_FEEDS) {
    try {
      ...
      for (const item of parsed.items) {
        ...
        const dedupKey = eventDedupKey(draft);
  ...
  if (!rawNew.length) {
    const cleaned = applyTimeWindow(existingEvents);
    if (cleaned.length !== existingRaw.length) {
      await store.set('events', cleaned);
      console.log(`[新闻] 无新文章，已去重清洗：${existingRaw.length} -> ${cleaned.length}`);
    } else {
      console.log('[新闻] 无新文章');
    }
    return cleaned;
  ...
  console.log(`[LLM] 翻译+分析 ${rawNew.length} 条，调用 DeepSeek...`);
  const analyzed = await analyzeWithDeepSeek(rawNew, existingEvents, deepseekToken);

  const merged = applyTimeWindow(dedupeEvents([...analyzed, ...existingEvents]));
  await store.set('events', merged);
  console.log(`[新闻] 新增 ${rawNew.length} 条，去重后共 ${merged.length} 条\n`);
  return merged;
}

// ─── Situation Report ─────────────────────────────────────────────────────────
const ANALYSIS_DEFAULT_MAX_AGE_MS = 6 * 60 * 60 * 1000;
 ...
 */
async function generateSituationReport(store, deepseekToken, options = {}) {
  const force = Boolean(options.force);
  const maxAgeMs = Number.isFinite(options.maxAgeMs)
    ? options.maxAgeMs
    : ANALYSIS_DEFAULT_MAX_AGE_MS;

  const existing = (await store.get('analysis')) || null;
  if (!force && existing?.lastUpdated) {
    ...
    if (Number.isFinite(lastTs) && (Date.now() - lastTs) < maxAgeMs) {
      ...
      console.log(`[分析] 跳过重算：上次更新 ${mins} 分钟前（阈值 ${Math.round(maxAgeMs / 3600000)} 小时）`);
      return existing;
    }
  }
  ...
  try {
    ...
    if (!res.ok) { console.error(`[分析] DeepSeek HTTP ${res.status}`); return existing; }

    const json    = await res.json();
    const content = json.choices?.[0]?.message?.content;
    if (!content) { console.error('[分析] 空响应'); return existing; }
